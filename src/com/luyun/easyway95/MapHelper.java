...
public class MapHelper {
	...
	// private SegmentTraffic mCurrentSegTraffic; //traffic received from tss,
	// used to update view of traffic list and update "traffic line??"
	...
	/*
	 * SegmentTraffic getSegTraffic() { return null; }
	 * 
	 * public boolean createTrafficPolylineInfo(SegmentTraffic segTraffic,
	 * RoadInfo roadInfo) { return true; }
	 * 
	 * public class RoadInfo { private String name; private List<PointHelper>
	 * points; }
	 */

	/*
	 * 核心功能：收到TSS发的路况信息，进行处理。 pacakge:msg, msg=[{name:r1,
	 * segments:[{startPoi:{lat, lng}, endPoi:{lat, lng}}, ...]}, ...]
	 * 客户端要有四个不同的容器： DrivingRoutes:
	 * 运动过程中如果有偏离航线的情况发生则更新，否则用于接收路况。对于每条路，新的时间点将覆盖以前的路况。
	 * RoadsInsight:周边的路况，也在运动过程中不断更新，每隔10分钟刷新一次，或者10KM更新一次
	 * HotRoadsWithTraffic：由系统设置，一般一个城市共享相同的信息，并且可以从服务器上生成图片传下来，类似于交通播报的概略图
	 * FavoriteRoads
	 * ：个性化设置，在用户设置完自己的家庭和办公室地址之后，自动将规划好的路径存到用户的preferences和profile里，并且上报到服务器
	 * 
	 * 登录用户和非登录用户的处理：
	 * 非登录用户：通过与TSS的消息将这四张表上报，用户标识是DEVICEID@deviceid.android.roadclouding
	 * ，这个标识也将用来标识ZMQ的ID
	 * 登录用户：用户标记为username@deviceid.ios.roadclouding，这个标识也用来标识ZMQ的ID
	 * 与TSS之间是否登录，每条命令都要通过携带Token，标识客户端是否登录
	 * TSS需要生成一张表，保存当前激活用户的Token，2小时之内如果没有收到任何消息则将这个Token和其对应的Profile删除
	 * ，新的Token来的时候，TSS到服务器上查询Profile
	 * Roadclouding和TSS需要一个机制，确定该Token是否有效，以及明确TSS是否直接存取数据库？
	 * 如果登录的话，要将用户名上报，由服务器从Profile中查询其listFavoriteRoads
	 * ，其用户名要做一个变换：username@roadclouding.com 否则TSS等待客户端上报
	 * 
	...
	public void onMsg(LYMsgOnAir msg) {
		...
		case LY_RET_CODE:
			if (msg.getRetCode() != LYRetCode.LY_SUCCESS) {
				Log.e(TAG,
						"some errors happend, error code=" + msg.getRetCode());
			}
			return;
		case LY_CHECKIN:
			String strUrl;
			...
			if (major > mainActivity.getMajorRelease()) {
				mainActivity.onSoftwareUpgrade(major, minor, strUrl, strDesc,
						true);
		...
		case LY_TRAFFIC_PUB:
			...
			if (mDrivingRoutes == null) {
				// the only situation happened when traffic_pub arrives prior to
				// driving routes instantiated, i.e, app reboots
				// just ignore it
				Log.e(TAG,
						"traffic_pub arrives before driving routes instantiated.");
				return;
			}
	...
	/*
	 * 位置更新时，首先判断是否偏离航线？YES则重新规划路径 与上次位置是否有10KM？YES则向百度发起周边路的POI查询，关键字：路，距离10KM
	...
	public void onLocationChanged(Location location) {
		...
		if (tr != null) {
			...
			LYMsgOnAir msg = com.luyun.easyway95.shared.TSSProtos.LYMsgOnAir
					...
					.setVersion(1)
					.setFromParty(
							com.luyun.easyway95.shared.TSSProtos.LYParty.LY_CLIENT)
					.setToParty(
							com.luyun.easyway95.shared.TSSProtos.LYParty.LY_TC)
					.setSndId(mainActivity.getDeviceID())
					.setMsgType(
							com.luyun.easyway95.shared.TSSProtos.LYMsgType.LY_TRAFFIC_REPORT)
					.setMsgId(2000)
					.setTimestamp(System.currentTimeMillis() / 1000)
					.setTrafficReport(tr).build();
		...
		// popup a prompt when distance between current location with traffic <
		// 10KM
		...
		if (mDrivingRoutes.isOnRoute(mCurrentPoint)) {
			Log.d(TAG, "on route");
			TrafficPoint nextTrafficPoint = mDrivingRoutes
					.getNextTrafficPoint(mCurrentPoint);
		...
		// STPointLineDistInfo stPointLineDistInfo = mDrivingRoutes.new
		// STPointLineDistInfo();
		// double distanceOffRoad = 0.0;
		// if (mDrivingRoutes != null) {
		// distanceOffRoad =
		// mDrivingRoutes.getNearestDistanceOfRoad(mCurrentPoint,
		// mDrivingRoutes.getAllPoints2(), stPointLineDistInfo);
		...
		// //on road
		// //从匹配后的拥堵点中找出下一个与当前距离
		// STPointLineDistInfo newSTPointLineDistInfo = mDrivingRoutes.new
		// STPointLineDistInfo();
		// Log.d(TAG, stPointLineDistInfo.toString());
		// ArrayList<GeoPoint> allMatchedPoints =
		// mDrivingRoutes.getMatchedPoints();
		// GeoPoint nextTrafficPoint = null;
		// double newDistanceOffRoad =
		// mDrivingRoutes.getNearestDistanceOfRoad(mCurrentPoint,
		// allMatchedPoints, newSTPointLineDistInfo);
		// if (Math.abs(newDistanceOffRoad) < Constants.DISTANCE_OFF_ROAD) {
		// //说明现在正在某段拥堵路中间
	...
	public void requestRoadsAround(Location currentLocation) {
		// GeoPoint point = new
		// GeoPoint((int)(currentLocation.getLatitude()*1E6),
	...
	public void requestRoadsAround(GeoPoint currentPoint) {
		...
		mkSearch.init(mBMapMan, new MKSearchListener() {
			public void onGetPoiResult(MKPoiResult res, int type, int error) {
				...
				if (res.getCurrentNumPois() > 0) {
					...
					Log.d(TAG,
							"found a lot of roads around. size="
		...
		mkSearch.poiSearchNearBy("路", currentPoint,
				Constants.CHKPNT_OF_DISTANCE * 1000);
	}

	public void requestDrivingRoutes(Location startLoc, Location endLoc) {
		GeoPoint startPoint = new GeoPoint(
				(int) (startLoc.getLatitude() * 1E6),
				(int) (startLoc.getLongitude() * 1E6));
		GeoPoint endPoint = new GeoPoint((int) (endLoc.getLatitude() * 1E6),
				(int) (endLoc.getLongitude() * 1E6));
	...
	private void requestDrivingRoutes(GeoPoint startPoint, GeoPoint endPoint) {
		Log.d(TAG, "enter requestDrivingRoutes, start=" + startPoint.toString()
		...
		mMKSearch.init(mBMapMan, new MKSearchListener() {
			...
			public void onGetDrivingRouteResult(MKDrivingRouteResult result,
				...
				// Log.d(TAG, "route number " +
				// result.getPlan(0).getNumRoutes());
				MKRoute route = result.getPlan(0).getRoute(0);
				// 这里将原来的规划路径覆盖
				mDrivingRoutes = new MKRouteHelper(route,
						mainActivity.mMapUtils);
				mainActivity.resetMapView();

				Log.d(TAG, "ArrayList<ArrayList<GeoPoint>> size..."
						+ route.getArrayPoints().size());

				// Iterator<ArrayList<GeoPoint>> itr =
				// route.getArrayPoints().iterator();
				// int index = 0;
				// while(itr.hasNext())
				// {
				// ArrayList<GeoPoint> arrayPoint = itr.next();
				// Log.d(TAG, "ArrayList<GeoPoint> index..." + index++);
				// Log.d(TAG, "ArrayList<GeoPoint> size..." +
				// arrayPoint.size());
				// Log.d(TAG, "ArrayList<GeoPoint> ..." +
				// arrayPoint.toString());
				// }
	...
	public void checkIn() {
		LYCheckin ci = com.luyun.easyway95.shared.TSSProtos.LYCheckin
				.newBuilder().setDeviceModel(android.os.Build.MODEL)
				.setOsType(LYOsType.LY_ANDROID)
				.setOsVersion(android.os.Build.VERSION.SDK)
				.setLyMajorRelease(mainActivity.getMajorRelease())
				.setLyMinorRelease(mainActivity.getMinorRelease()).build();
		...
		LYMsgOnAir msg = com.luyun.easyway95.shared.TSSProtos.LYMsgOnAir
				...
				.setSndId(mainActivity.getDeviceID())
				.setMsgType(
						com.luyun.easyway95.shared.TSSProtos.LYMsgType.LY_CHECKIN)
				.setMsgId(2000).setTimestamp(System.currentTimeMillis() / 1000)
				.setCheckin(ci).build();
	...
	private void onCronpub(LYTrafficPub trafficPub) {
		...
		// (NotificationManager)mainActivity.getSystemService(ns);
		//
		// Intent intent = new Intent(mainActivity, ShowTraffics.class);
		// intent.putExtra("cronpub", true);
		// PendingIntent pendIntent = PendingIntent.getActivity(mainActivity, 0,
		// intent, PendingIntent.FLAG_ONE_SHOT);

		String title = "路易95实时路况";
		String content = "";

		if (trafficPub.hasCityTraffic()) {
			for (int rd = 0; rd < trafficPub.getCityTraffic()
					.getRoadTrafficsCount(); rd++) {
				TSSProtos.LYRoadTraffic tf = trafficPub.getCityTraffic()
						.getRoadTraffics(rd);
				content += tf.getRoad();

				Log.d(TAG,
						"desc: " + tf.getDesc() + ", segment count: "
								+ tf.getSegmentTrafficsCount());

				String seginf = "";
				for (int seg = 0; seg < tf.getSegmentTrafficsCount(); seg++) {
					TSSProtos.LYSegmentTraffic segtraffic = tf
							.getSegmentTraffics(seg);

					if (segtraffic.hasDetails())
						seginf += segtraffic.getDetails();
					// if(segtraffic.hasDirection()) seginf +=
					// segtraffic.getDirection();

					int speed = segtraffic.getSpeed();
					String strSpeed = Constants.TRAFFIC_JAM_LVL_HIGH;
					if (speed >= Constants.TRAFFIC_JAM_LVL_MIDDLE_SPD)
						strSpeed = Constants.TRAFFIC_JAM_LVL_LOW;
					if (speed < Constants.TRAFFIC_JAM_LVL_MIDDLE_SPD
							&& speed >= Constants.TRAFFIC_JAM_LVL_HIGH_SPD)
						strSpeed = Constants.TRAFFIC_JAM_LVL_MIDDLE;

					if (segtraffic.hasSpeed())
						seginf += strSpeed;
				}
		...
		notification = new NotificationData(mainActivity);
		notification.SetNotification(R.drawable.icon95, title, content,
				ShowTraffics.class);
	}
}

// added by chenfeng on 2013-1-7
class NotificationData {
	...
	public void SetNotification(int drawable, String title, String content,
			Class cls) {
		mNotificationManager = (NotificationManager) _context
				.getSystemService(Context.NOTIFICATION_SERVICE);
		final Notification notifyDetails = new Notification(drawable, content,
		...
		// notifyIntent.putExtra("cronpub", true);
